...
class LayerNorm(paddle.nn.Layer):
    ...
    def __init__(self, hidden_size, eps=1e-12):
        ...
        out_4 = paddle.create_parameter(shape=paddle.to_tensor(data=hidden_size).shape, 
                                        dtype = 'float32',
        ...
        self.weight = out_4
        out_5 = paddle.create_parameter(shape=paddle.to_tensor(data=hidden_size).shape, 
                                        dtype = 'float32',
                                        # dtype=paddle.to_tensor(data=hidden_size).numpy().dtype, 
                                        default_initializer=paddle.nn.initializer.Assign(paddle.to_tensor(data=hidden_size)))
