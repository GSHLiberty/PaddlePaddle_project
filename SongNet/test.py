...
def init_model(m_path, device, vocab):
    ...
    lm_model = BIGLM(device, lm_vocab, lm_args.embed_dim, lm_args.
        ff_embed_dim, lm_args.num_heads, lm_args.dropout, lm_args.layers, 0.1)
...
# m_path = '/data2/gsh/paddlepaddle/SongNet/convert_ckpt/song.ckpt'
m_path = './model/epoch31_batch_79999'
...
def top_k_inc(enc, src_padding_mask, inp_ys_tpl, inp_ys_seg, inp_ys_pos, s):
    ...
    inp_y = inp_y
    res = []
    for l in range(inp_ys_tpl.shape[0]):
        probs, pred, incremental_state = lm_model.work_incremental(enc,
            src_padding_mask, inp_y, inp_ys_tpl[0:l + 1, :], inp_ys_seg[0:l +
            1, :], inp_ys_pos[0:l + 1, :], incremental_state)
        next_tk = []
        ...
        inp_y, m = s2t(s, lm_vocab)
        inp_y = inp_y
        bidx = paddle.to_tensor(data=bidx, dtype='bool')
...
def top_k(enc, src_padding_mask, inp_ys_tpl, inp_ys_seg, inp_ys_pos, s):
    ...
    for l in range(inp_ys_tpl.shape[0]):
        probs, pred = lm_model.work(enc, src_padding_mask, inp_y,
            inp_ys_tpl[0:l + 1, :], inp_ys_seg[0:l + 1, :], inp_ys_pos[0:l +
            1, :])
        next_tk = []
        for i in range(len(s)):
            ctk = lm_vocab.idx2token(inp_ys_tpl[l, i].item())
...
def beam_decode(s, x, enc, src_padding_mask, inp_ys_tpl, inp_ys_seg, inp_ys_pos
    ...
    last_traces = [[]]
    last_scores = paddle.to_tensor(data=np.zeros(1), dtype='float32').to(gpu)
    x = x.to(gpu)
    ys = x
    for l in range(inp_ys_tpl.shape[0]):
        seq_len, bsz = ys.shape
        ...
        y_pred, _ = lm_model.work(enc_, src_padding_mask_, ys, inp_ys_tpl_[
            0:l + 1, :], inp_ys_seg_[0:l + 1, :], inp_ys_pos_[0:l + 1, :])
        ...
        last_scores = paddle.to_tensor(data=np.array(last_scores).reshape((
            num_live, 1)), dtype='float32').to(gpu)
        next_y = []
        for e in last_traces:
            eid = e[-1].item()
            next_y.append(eid)
        next_y = np.array(next_y).reshape((1, num_live))
        next_y = paddle.to_tensor(data=next_y, dtype='int64').to(gpu)
...
for i in range(5):
    idx = 0
    fo = open('./results/new/top-' + str(k) + '/out' + str(i + 1) + '.txt', 'w')
    while idx < len(ds):
