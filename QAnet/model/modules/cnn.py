    ...
    def __init__(self, in_ch, out_ch, k, dim=1, bias=False):
        ...
        if dim == 1:
            self.depthwise_conv = paddle.nn.Conv1D(in_channels=in_ch,
                out_channels=in_ch, kernel_size=k, groups=in_ch, padding=k //
                2, bias_attr=bias)
            self.pointwise_conv = paddle.nn.Conv1D(in_channels=in_ch,
                out_channels=out_ch, kernel_size=1, padding=0, bias_attr=bias)
        elif dim == 2:
            self.depthwise_conv = paddle.nn.Conv2D(in_channels=in_ch,
                out_channels=in_ch, kernel_size=k, groups=in_ch, padding=k //
                2, bias_attr=bias)
            self.pointwise_conv = paddle.nn.Conv2D(in_channels=in_ch,
                out_channels=out_ch, kernel_size=1, padding=0, bias_attr=bias)
        else:
