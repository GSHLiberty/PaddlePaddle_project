...
import json, re
path = PATH()
ques_dict_preread = {'train': json.load(open(path.RAW_PATH['gqa']['train'],
    'r')), 'val': json.load(open(path.RAW_PATH['gqa']['val'], 'r')),
    'testdev': json.load(open(path.RAW_PATH['gqa']['testdev'], 'r')),
    'test': json.load(open(path.RAW_PATH['gqa']['test'], 'r'))}
stat_ques_dict = {**ques_dict_preread['train'], **ques_dict_preread['val'],
    **ques_dict_preread['testdev'], **ques_dict_preread['test']}
stat_ans_dict = {**ques_dict_preread['train'], **ques_dict_preread['val'],
...
def tokenize(stat_ques_dict):
    ...
    for qid in stat_ques_dict:
        ...
        words = re.sub('([.,\'!?\\"()*#:;])', '', ques.lower()).replace('-',
            ' ').replace('/', ' ').split()
...
json.dump([ans_to_ix, ix_to_ans, token_to_ix, max_token], open(
    '../openvqa/datasets/gqa/dicts.json', 'w'))
