...
This repository corresponds to the PaddlePaddle implementation of the BAN for VQA.
...
```bash
$ python3 run.py --RUN='train' --MODEL='ban_4' --DATASET='vqa'
```
- ``--RUN={'train','val','test'}`` to set the mode to be executed.
- ``--MODEL={'ban_4','ban-8'``}, e.g., to assign the model to be executed.
...
We provide two pretrained model, namely the `ban_4` and `ban_8` model.  


The model can be downloaded from  [BaiduYun](https://pan.baidu.com/s/1dl_fgaaW6k-Eqq4Vp7WRQA),  keyword is : urt9. You should unzip and put them to the correct folders as follows:

```angular2html
|-- ckpt
	|-- ckpt_small
	|  |-- ban_4_model_full.pdparams
	|-- ckpt_large
	|  |-- ban_8_model_full.pdparams
```
