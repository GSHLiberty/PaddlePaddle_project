...
def eval(__C, dataset, ans_ix_list, pred_list, result_eval_file,
    ensemble_file, log_file, valid=False):
    ...
    if __C.TEST_SAVE_PRED:
        ...
        result_pred = [{'pred': pred_list[qix], 'qid': qix} for qix in
            range(dataset.data_size)]
    ...
    if valid:
        ...
        assert num_true == num_pred, 'Expected %d answers but got %d' % (
            num_true, num_pred)
        for i, (true_answer, predicted_answer) in enumerate(zip(
        ...
        score_type = ['Overall', 'Count', 'Exist', 'Compare_Numbers',
            'Query_Attribute', 'Compare_Attribute']
        compare_numbers_type = ['greater_than', 'less_than']
        query_attribute_type = ['query_color', 'query_material',
            'query_shape', 'query_size']
        compare_attribute_type = ['equal_color', 'equal_integer',
