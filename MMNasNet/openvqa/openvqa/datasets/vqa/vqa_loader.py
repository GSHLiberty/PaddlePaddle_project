...
class DataSet(BaseDataSet):
    ...
    def __init__(self, __C):
        ...
        frcn_feat_path_list = glob.glob(__C.FEATS_PATH[__C.DATASET]['train'
            ] + '/*.npz') + glob.glob(__C.FEATS_PATH[__C.DATASET]['val'] +
            '/*.npz') + glob.glob(__C.FEATS_PATH[__C.DATASET]['test'] +
            '/*.npz')
    ...
    def load_img_feats(self, idx, iid):
        ...
        frcn_feat_iter = self.proc_img_feat(frcn_feat_x, img_feat_pad_size=
            self.__C.FEAT_SIZE['vqa']['FRCN_FEAT_SIZE'][0])
        bbox_feat_iter = self.proc_img_feat(self.proc_bbox_feat(frcn_feat[
            'bbox'], (frcn_feat['image_h'], frcn_feat['image_w'])),
