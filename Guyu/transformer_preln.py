...
class MultiheadAttention(paddle.nn.Layer):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, weights_dropout=True
        ...
        out_2 = paddle.create_parameter(shape=paddle.empty(shape=[3 *
            embed_dim, embed_dim]).shape, dtype=paddle.empty(shape=[3 *
            embed_dim, embed_dim]).numpy().dtype, default_initializer=
            paddle.nn.initializer.Assign(paddle.empty(shape=[3 * embed_dim,
            embed_dim])))
        out_2.stop_gradient = not True
        self.in_proj_weight = out_2
        out_3 = paddle.create_parameter(shape=paddle.to_tensor(data=3 *
            embed_dim, dtype='float32').shape, dtype=paddle.to_tensor(data=
            3 * embed_dim, dtype='float32').numpy().dtype,
            default_initializer=paddle.nn.initializer.Assign(paddle.
            to_tensor(data=3 * embed_dim, dtype='float32')))
        out_3.stop_gradient = not True
        self.in_proj_bias = out_3
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=
        ...
        if incremental_state is not None:
            ...
            saved_state = None
            bidx = None
        qkv_same = (query.shape == key.shape) and (query.shape == value.shape)
        ...
        perm_10[1] = 2
        perm_10[2] = 1
        attn_weights = paddle.bmm(x=q, y=x.transpose(perm=perm_10))
        ...
        perm_12[0] = 1
        perm_12[1] = 0
        attn = paddle.reshape(x.transpose(perm=perm_12), shape=[tgt_len, bsz, embed_dim])
        ...
        if need_weights:
            ...
            perm_13[0] = 1
            perm_13[1] = 0
            attn_weights = x.transpose(perm=perm_13)
        else:
            attn_weights = None
