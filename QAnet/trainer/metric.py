    ...
    with open(dataset_file) as dataset_file:
        ...
        if dataset_json['version'] != expected_version:
            print('Evaluation expects v-' + expected_version +
                ', but got dataset with v-' + dataset_json['version'], file
                =sys.stderr)
...
def em_by_begin_end_index(pred_begin, pred_end, begin, end):
    ...
    exact_correct_num = paddle.sum(x=(pred_begin == begin) * (pred_end == end))
...
def em_by_begin_end_index_max(pred_begin, pred_end, begins, ends):
    ...
    for i in range(batch_size):
        ...
        for j in range(num_answers):
            em.append((pred_begin[i] == begins[i][j]) * (pred_end[i] ==
                ends[i][j]))
