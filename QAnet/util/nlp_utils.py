...
def load_w2v(fin, binary, vector_size, specials=['<UNK>', '<PAD>', '<SOS>',
    '<EOS>']):
    """
    ...
    if specials is not None:
        for special in specials:
            ...
            vocab.add(y=paddle.to_tensor(special))
    wv = OrderedDict(wv)
    wv_word2ix = {word: i for i, word in enumerate(wv)}
    wv_dim = vector_size
    wv_tensor = paddle.to_tensor(list(wv.values()), dtype='float32').reshape([-1, wv_dim])
    wv_vocab = vocab
    return wv_tensor, wv_word2ix, wv_vocab, wv_dim


def read_w2v(fin, binary, vector_size, specials=['<UNK>', '<PAD>', '<SOS>',
    '<EOS>'], cache_file=None):
    """
    ...
    if os.path.isfile(cache_file):
        ...
        wv_tensor, wv_word2ix, wv_vocab, wv_dim = pickle_load_large_file(
            cache_file)
        print('Finished reading word vector from %s' % cache_file)
    else:
        print('Read word vector')
        wv_tensor, wv_word2ix, wv_vocab, wv_dim = load_w2v(fin, binary,
            vector_size, specials)
...
if __name__ == '__main__':
    ...
    wv_tensor, wv_word2ix, wv_vocab, wv_dim = load_w2v(
        ...
        , True, 300, specials=['<UNK>', '<PAD>', '<SOS>', '<EOS>'])
    print(wv_tensor.shape)
    print(len(wv_vocab))
    print(wv_dim)
