# Multi-modal Factorized Bilinear Pooling with Co-Attention Learning for Visual Question Answering (MFB)

This repository corresponds to the PaddlePaddle implementation of the MFB for VQA.
...
```bash
$ python3 run.py --RUN='train' --MODEL='mfb' --DATASET='vqa'
```
...
We provide a pretrained model, namely the `mfb` model.  


The model can be downloaded from  [BaiduYun](https://pan.baidu.com/s/1tx18WGk2AufcMbLHBmTcmg),  keyword is : 5auy. You should unzip and put them to the correct folders as follows:

```angular2html
|-- ckpt
	|-- mfb_model_full.pdparams
```
