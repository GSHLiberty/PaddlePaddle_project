# Beyond Bilinear: Generalized Multimodal Factorized High-order Pooling for Visual Question Answering (MFH)

This repository corresponds to the PaddlePaddle implementation of the MFH for VQA.
...
```bash
$ python3 run.py --RUN='train' --MODEL='mfh' --DATASET='vqa'
```
...
We provide a pretrained model, namely the `mfh` model.  


The model can be downloaded from  [BaiduYun](https://pan.baidu.com/s/1nL_lzupedrIJxL67orqGaQ),  keyword is : 6lmt. You should unzip and put them to the correct folders as follows:

```angular2html
|-- ckpt
	|-- mfh_model_full.pdparams
```
