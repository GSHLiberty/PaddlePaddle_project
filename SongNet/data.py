...
TS = ['<t-1>'] + [('<t' + str(i) + '>') for i in range(32)]
PUNCS = set([',', '.', '?', '!', ':', '，', '。', '？', '！', '：'])
BUFSIZE = 4096000
...
def batchify(data, vocab):
    ...
    xs_tpl = paddle.to_tensor(ListsToTensor(xs_tpl, vocab), dtype='int64')
    xs_tpl = paddle.transpose(xs_tpl, [1, 0])
    xs_seg = paddle.to_tensor(data=ListsToTensor(xs_seg, vocab), dtype='int64')
    xs_seg = paddle.transpose(xs_seg, [1, 0])
    xs_pos = paddle.to_tensor(data=ListsToTensor(xs_pos, vocab), dtype='int64')
    xs_pos = paddle.transpose(xs_pos, [1, 0])
    ys_truth = paddle.to_tensor(data=ListsToTensor(ys_truth, vocab), dtype='int64')
    ys_truth = paddle.transpose(ys_truth, [1, 0])
    ys_inp = paddle.to_tensor(data=ListsToTensor(ys_inp, vocab), dtype='int64')
    ys_inp = paddle.transpose(ys_inp, [1, 0])
    ys_tpl = paddle.to_tensor(data=ListsToTensor(ys_tpl, vocab), dtype='int64')
    ys_tpl = paddle.transpose(ys_tpl, [1, 0])
    ys_seg = paddle.to_tensor(data=ListsToTensor(ys_seg, vocab), dtype='int64')
    ys_seg = paddle.transpose(ys_seg, [1, 0])
    ys_pos = paddle.to_tensor(data=ListsToTensor(ys_pos, vocab), dtype='int64')
    ys_pos = paddle.transpose(ys_pos, [1, 0])
...
class Vocab(object):
    ...
    def __init__(self, filename, min_occur_cnt, specials=None):
        idx2token = [PAD, UNK, BOS, EOS] + [BOC, EOC, LS, RS, SP
            ] + CS + SS + PS + TS + (specials if specials is not None else [])
