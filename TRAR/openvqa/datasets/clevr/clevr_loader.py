...
class DataSet(BaseDataSet):
    ...
    def __init__(self, __C):
        ...
        stat_ques_list = json.load(open(__C.RAW_PATH[__C.DATASET]['train'],
            ...
            DATASET]['test'], 'r'))['questions']
        stat_ans_list = json.load(open(__C.RAW_PATH[__C.DATASET]['train'], 'r')
            )['questions'] + json.load(open(__C.RAW_PATH[__C.DATASET]['val'
            ], 'r'))['questions']
        self.ques_list = []
        grid_feat_path_list = []
        ...
        for split in split_list:
            self.ques_list += json.load(open(__C.RAW_PATH[__C.DATASET][
                split], 'r'))['questions']
            grid_feat_path_list += glob.glob(__C.FEATS_PATH[__C.DATASET][
                split] + '/*.npz')
        self.data_size = self.ques_list.__len__()
        print(' ========== Dataset size:', self.data_size)
        self.iid_to_grid_feat_path = self.img_feat_path_load(
            grid_feat_path_list)
        self.token_to_ix, self.pretrained_emb, max_token = self.tokenize(
    ...
    def load_ques_ans(self, idx):
        ...
        ques_ix_iter = self.proc_ques(ques, self.token_to_ix, max_token=
            self.max_token)
        ans_iter = np.zeros(1)
