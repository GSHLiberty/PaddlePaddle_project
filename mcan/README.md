...
This repository corresponds to the PaddlePaddle implementation of the MCAN for VQA, which won the champion in [VQA Challgen 2019](https://visualqa.org/roe.html). 
...
We provide two pretrained models, namely the `small` model and the `large` model.  The large model uses a 2x larger `HIDDEN_SIZE=1024` compared to the small model with `HIDDEN_SIZE=512`. 


These two models can be downloaded from  [BaiduYun](https://pan.baidu.com/s/10szbutbMM_dDzm2SwCGNow),  keyword is : ufc4. You should unzip and put them to the correct folders as follows:
